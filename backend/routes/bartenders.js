...
router.get("/success", async function (req, res, next) {
  try {
    let [rows, fields] = await pool.query(
      `SELECT h.id_order,p.product_name,h.amount,h.type,h.at_date FROM history_order as h LEFT JOIN products as p ON h.id_product =p.id_product WHERE h.process = 'D'`
    );
    console.log("hello");
    return res.status(200).send(rows);
  } catch (e) {
